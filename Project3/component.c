...
int instruction_decode(unsigned op, struct_controls *controls)
{
    ...
    // unconditional jump
        ...
        return 1; // invalid instruction or 'halt' condition occurs
    return 0;
}
...
int ALU_operations(unsigned data1, unsigned data2, unsigned extended_value, unsigned funct, char ALUOp, char ALUSrc, unsigned *ALUresult, char *Zero)
{
    ...
    case 7:
        ...
        if (funct == 0x20)
            ...
            ALU(data1, data2, 0x2, ALUresult, Zero); // set less than (signed)
        else if (funct == 0x2B)
            ALU(data1, data2, 0x3, ALUresult, Zero); // set less than (unsigned)
        else
...
{
    if (ALUresult >= 0x4000 && ALUresult <= 0xFFFF && (ALUresult % 4 == 0))
    {
        if (MemRead == 1)
